...
def heatmap(df,state_column_name,outputcolumn,cmap="plasma"):
    ...
    merge_with=df[[state_column_name,outputcolumn]].copy()
    merge_with[state_column_name]=merge_with[state_column_name].str.upper()
...
def regression(df,col1,col2):
    ...
    print(f"The correlation between {col1} and {col2} is {model.rvalue:.2f} with a p-value of {model.pvalue}")
